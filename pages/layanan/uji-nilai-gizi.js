...
export default function UjiNilaiGiziPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log(`✅ Uji Nilai Gizi loaded: ${result.data.length} records (${result.source || 'unknown'} source)`)
      } else {
        console.error('Error loading Uji Nilai Gizi data:', result.error)
        setDataList([])
      }
    } catch (error) {
      console.error('Error loading Uji Nilai Gizi data:', error)
      setDataList([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchIkmBinaanData = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log(`✅ IKM Binaan loaded: ${result.data.length} records for Uji Nilai Gizi selection`)
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        await fetchData()
        resetForm()
        alert(editingId ? 'Uji Nilai Gizi berhasil diperbarui!' : 'Uji Nilai Gizi berhasil disimpan!')
      } else {
        alert('Gagal menyimpan data: ' + result.message)
      }
    } catch (error) {
      console.error('Error saving Uji Nilai Gizi data:', error)
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...
      nomor_lhu: item.nomor_lhu || '',
      tanggal_hasil_uji: item.tanggal_hasil_uji || '',
      tahun_fasilitasi: item.tahun_fasilitasi || '2026',
      link_lhu: item.link_lhu || '',
    })
    setShowForm(true)
  }
  ...
  const handleDelete = async (id) => {
    if (confirm('Yakin ingin menghapus data ini?')) {
      try {
        ...
        console.error('Error deleting Uji Nilai Gizi data:', error)
  ...
  const filteredDataList = dataList.filter(item =>
    ...
    (item.nomor_lhu || '').toLowerCase().includes(searchTerm.toLowerCase())
  )
        ...
        <Layout>
          <div className="flex items-center justify-center min-h-screen">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-600">Memuat data Uji Nilai Gizi...</p>
            </div>
          </div>
        </Layout>
      </AdminRoute>
  ...
  return (
    <AdminRoute>
      <Layout>
        <div className="p-8">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-gray-900">Uji Nilai Gizi</h1>
            <p className="text-gray-600">Kelola data Uji Nilai Gizi untuk IKM Binaan</p>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Cari data Uji Nilai Gizi..."
                />
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nomor LHU</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tanggal Hasil Uji</th>
                ...
                {filteredDataList.length > 0 ? (
                  filteredDataList.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.nomor_lhu || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {item.tanggal_hasil_uji ? new Date(item.tanggal_hasil_uji).toLocaleDateString('id-ID') : '-'}
                          </td>
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-12 text-center text-gray-500">
                      {searchTerm ? 'Tidak ada data yang sesuai dengan pencarian' : 'Belum ada data Uji Nilai Gizi'}
                    </td>
                  </tr>
                )}
              </tbody>
