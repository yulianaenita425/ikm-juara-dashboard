...
export default function SIINasPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log(`✅ SIINas loaded: ${result.data.length} records (${result.source || 'unknown'} source)`)
      } else {
        console.error('Error loading SIINas data:', result.error)
        setDataList([])
      }
    } catch (error) {
      console.error('Error loading SIINas data:', error)
      setDataList([])
    } finally {
      setLoading(false)
    }
  ...
  const fetchIkmBinaanData = async () => {
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log(`✅ IKM Binaan loaded: ${result.data.length} records for SIINas selection`)
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        await fetchData()
        resetForm()
        alert(editingId ? 'SIINas berhasil diperbarui!' : 'SIINas berhasil disimpan!')
      } else {
        alert('Gagal menyimpan data: ' + result.message)
      }
    } catch (error) {
      console.error('Error saving SIINas data:', error)
  ...
  const handleEdit = (item) => {
    ...
    setFormData({
      ...
      nomor_bukti_akun: item.nomor_bukti_akun || '',
      tahun_registrasi: item.tahun_registrasi || '2026',
  ...
  const handleDelete = async (id) => {
    if (confirm('Yakin ingin menghapus data ini?')) {
      try {
        ...
        console.error('Error deleting SIINas data:', error)
  ...
  const filteredDataList = dataList.filter(item =>
    ...
    (item.nomor_bukti_akun || '').toLowerCase().includes(searchTerm.toLowerCase())
  )
        ...
        <Layout>
          <div className="flex items-center justify-center min-h-screen">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-600">Memuat data SIINas...</p>
            </div>
          </div>
        </Layout>
      </AdminRoute>
  ...
  return (
    <AdminRoute>
      <Layout>
        <div className="p-8">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-gray-900">SIINas</h1>
            <p className="text-gray-600">Kelola data SIINas untuk IKM Binaan</p>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nomor Bukti Akun</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tahun Registrasi</th>
                ...
                {filteredDataList.length > 0 ? (
                  filteredDataList.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.nomor_bukti_akun || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{item.tahun_registrasi || '-'}</td>
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-12 text-center text-gray-500">
                      {searchTerm ? 'Tidak ada data yang sesuai dengan pencarian' : 'Belum ada data SIINas'}
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFormData({...formData, tahun_registrasi: e.target.value})}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          required
                        />
                      </div>
