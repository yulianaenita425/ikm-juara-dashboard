  ...
  if (req.method === 'GET') {
    try {
      ...
      if (supabaseAdmin) {
        try {
          ...
          data = realData || []
          console.log(`✅ SIINas: Loaded ${data.length} records from database`)
          
        } catch (dbError) {
          console.warn(`❌ SIINas database error, using fallback:`, dbError.message)
          data = getFallbackData()
        }
      } else {
        console.log(`⚠️ SIINas: No database configured, using fallback data`)
        data = getFallbackData()
      }
      ...
      console.error(`Error in SIINas API:`, error)
    ...
    try {
      ...
      res.status(201).json({ 
        success: true, 
        data: data[0],
        message: 'SIINas berhasil ditambahkan'
      })
    ...
    try {
      ...
      res.status(200).json({ 
        success: true, 
        data: data[0],
        message: 'SIINas berhasil diperbarui'
      })
    ...
    try {
      ...
      res.status(200).json({ 
        success: true,
        message: 'SIINas berhasil dihapus'
      })
