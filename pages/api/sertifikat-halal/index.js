  ...
  if (req.method === 'GET') {
    try {
      ...
      if (supabaseAdmin) {
        try {
          ...
          data = realData || []
          console.log(`✅ Sertifikat Halal: Loaded ${data.length} records from database`)
          
        } catch (dbError) {
          console.warn(`❌ Sertifikat Halal database error, using fallback:`, dbError.message)
          data = getFallbackData()
        }
      } else {
        console.log(`⚠️ Sertifikat Halal: No database configured, using fallback data`)
        data = getFallbackData()
      }
      ...
      console.error(`Error in Sertifikat Halal API:`, error)
    ...
    try {
      ...
      res.status(201).json({ 
        success: true, 
        data: data[0],
        message: 'Sertifikat Halal berhasil ditambahkan'
      })
      
    } catch (error) {
      console.error(`Error creating Sertifikat Halal:`, error)
      res.status(500).json({ 
        success: false, 
    ...
    try {
      ...
      res.status(200).json({ 
        success: true, 
        data: data[0],
        message: 'Sertifikat Halal berhasil diperbarui'
      })
      
    } catch (error) {
      console.error(`Error updating Sertifikat Halal:`, error)
      res.status(500).json({ 
        success: false, 
        message: error.message 
      })
    ...
    try {
      ...
      res.status(200).json({ 
        success: true,
        message: 'Sertifikat Halal berhasil dihapus'
      })
