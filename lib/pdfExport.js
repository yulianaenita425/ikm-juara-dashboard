...
// Export Peserta Pelatihan Spesifik dengan Detail Lengkap
export const exportPesertaPelatihanPDF = async (pesertaData, jenisNama, jenisDetail = null) => {
  try {
    const PDFClass = await loadPDFLibraries()
    if (!PDFClass) throw new Error('PDF library tidak tersedia')
    ...
    // Header
    let yPos = addHeader(`Detail Peserta ${jenisNama}`, 'DisnakerKUKM Kota Madiun')
    
    // Info pelatihan detail jika tersedia
    if (jenisDetail) {
      doc.setFontSize(12)
      doc.setFont('helvetica', 'bold')
      doc.text('Rincian Jenis Kegiatan', 20, yPos)
      yPos += 10
      
      doc.setFontSize(9)
      doc.setFont('helvetica', 'normal')
      doc.text(`Jenis Pelatihan: ${jenisDetail.jenis_pelatihan}`, 20, yPos)
      yPos += 5
      doc.text(`Sub Kegiatan: ${jenisDetail.sub_kegiatan}`, 20, yPos)
      yPos += 5
      doc.text(`Waktu Pelaksanaan: ${jenisDetail.waktu_pelaksanaan}`, 20, yPos)
      yPos += 5
      doc.text(`Tempat Pelaksanaan: ${jenisDetail.tempat}`, 20, yPos)
      yPos += 5
      doc.text(`Tahun: ${jenisDetail.tahun_pelaksanaan}`, 20, yPos)
      yPos += 15
    } else {
      doc.setFontSize(10)
      doc.setFont('helvetica', 'normal')
    ...
    // Prepare table data sesuai permintaan: No, NIB, NIK, Nama Lengkap, Nama Usaha, No. Telp
    const tableData = pesertaData.map((item, index) => [
      index + 1,
      item.ikm_binaan?.nib || item.nib || '-',
      item.ikm_binaan?.nik || item.nik || '-',
    ...
    doc.autoTable({
      startY: yPos,
      head: [['No', 'NIB', 'NIK', 'Nama Lengkap', 'Nama Usaha', 'No. Telp', 'Tanggal', 'Sertifikat']],
      body: tableData,
      styles: {
        fontSize: 7,
        cellPadding: 2,
      ...
      columnStyles: {
        ...
        1: { cellWidth: 25 }, // NIB
        2: { cellWidth: 25 }, // NIK
        3: { cellWidth: 35 }, // Nama Lengkap
        4: { cellWidth: 35 }, // Nama Usaha
        5: { cellWidth: 25 }, // No. Telp
        6: { cellWidth: 20 }, // Tanggal
        7: { cellWidth: 20 }, // Sertifikat
      },
      margin: { left: 20, right: 20 },
      didDrawPage: function(data) {
        addFooter()
    ...
    // Save PDF
    const fileName = `Detail_Peserta_${jenisNama.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`
    doc.save(fileName)
